...
class Topology:
    ...
    # ------------------------------------------------------------------
    # Position computation (vectorised over all satellites)
    # ------------------------------------------------------------------
    def _compute_all_positions(self):
        """Compute lat/lon/xyz for all satellites at self.time_t."""
        cfg = self.config
        ...
        for p in range(cfg.planes):
            ...
            for s in range(cfg.sat_per_plane):
                ...
                theta = ((s / cfg.sat_per_plane) * 2 * math.pi
                         + phase_offset
                         + omega * self.time_t)

                # Geocentric lat/lon
                lat_rad = math.asin(math.sin(incl) * math.sin(theta))
                lon_rad = (math.atan2(math.cos(incl) * math.sin(theta), math.cos(theta)) + raan - cfg.earth_rot_speed * self.time_t)
    ...
    def _build_edges(self):
        ...
        for p in range(cfg.planes):
            for s in range(cfg.sat_per_plane):
                ...
                self.graph.add_edge(nid, next_id,
                                    capacity=cfg.C_e, delay=delay)
                self.graph.add_edge(next_id, nid,
                                    capacity=cfg.C_e, delay=delay)
                ...
                if p < cfg.planes - 1:
                    if (abs(self._lats[nid]) <= cfg.lat_cutoff):
                        ...
                        if abs(self._lats[right_id]) <= cfg.lat_cutoff:
                            ...
                            self.graph.add_edge(nid, right_id,
                                                capacity=cfg.C_e, delay=delay)
                            self.graph.add_edge(right_id, nid,
                                                capacity=cfg.C_e, delay=delay)
    ...
    def _map_ground_stations(self):
        ...
        for gw in self.gateways:
            ...
            a = (np.sin(dlat / 2)**2
                 + math.cos(gw_lat) * np.cos(sat_lats_rad)
                 * np.sin(dlon / 2)**2)
    ...
    def _ground_haversine(self, lat1, lon1, lat2, lon2) -> float:
        ...
        return self.config.earth_radius * 2 * math.atan2(
    ...
    def get_server_sat_ids(self) -> list:
        ...
        return [nid for nid, d in self.graph.nodes(data=True)
                if d['has_server']]
